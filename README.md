...
find_relevant_docs("бюджет", top_k=3)
ask_question("Какой годовой бюджет проекта?")
```

## Проверочные факты

В `sample_docs/` вшиты специфичные факты, которых **нет в интернете** — по ним
преподаватель убеждается, что RAG читает именно нашу базу, а не «фантазирует».

| Факт | Значение |
|------|----------|
| Кодовое имя проекта | **Гидра-7** |
| Годовой бюджет | **42 попугая** |
| Ведущий инженер | **Марфа Кузнецова** |
| Дата-центр | **Тундра-9** |
| Кодовое имя релиза | **Аметист 3.14** |
| Пиковая пропускная способность | **9000 квазаров/с** |
| Порт координатора кластера | **47281** |
| Кодовое слово доступа | **«малахитовый барсук»** |

Контрольные вопросы для проверки:

- «Кто ведущий инженер проекта?» → *Марфа Кузнецова*
- «Какой годовой бюджет?» → *42 попугая*
- «В каком дата-центре размещён проект?» → *Тундра-9*
