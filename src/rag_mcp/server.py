...
mcp = FastMCP(
    "rag-kb",
    instructions=(
        "База знаний ИПК им. А.Н. Квадратова — "
        "вымышленного института плоскостной космологии. "
        "Содержит внутренние документы: персоналии, "
        "программы наблюдений, технические протоколы, "
        "исторические архивы института. "
        "Используй для вопросов об институте, его сотрудниках и исследованиях. "
        "Перед поиском убедись, что индекс наполнен (index_status), "
        "иначе сначала вызови index_folder."
    ),
)
...
def ask_question(query: str) -> dict:
    """Ответить на вопрос по документам через Corrective RAG-пайплайн.

    Используй для любых вопросов по содержимому базы знаний.
