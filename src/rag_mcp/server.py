...
mcp = FastMCP(
    "rag-kb",
    instructions=(
        "База знаний проектной документации. Содержит технические спецификации, "
        "инфраструктуру, персонал (имена, роли), кодовые названия проектов, "
        "бюджеты, даты релизов, параметры систем и факты из внутренних документов. "
        "Используй, когда пользователь спрашивает о конкретных проектах, "
        "сотрудниках, технических характеристиках или кодовых именах."
    ),
)
...
def ask_question(query: str) -> dict:
    """Ответить на вопрос по проектной документации через Corrective RAG-пайплайн.
    ...
    (до 2 повторов) → генерация ответа локальной LLM (Ollama). Используй для
    вопросов о: кодовых именах проектов, именах и ролях сотрудников, бюджетах,
    датах релизов, технических характеристиках, параметрах инфраструктуры.
